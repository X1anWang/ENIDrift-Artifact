...
class dA:
    ...
    def train(self, x):
        ...
        for iii in range(self.params.n_visible):
            if self.norm_max[iii] < x[0,iii]:
                self.norm_max[iii] = x[0,iii]
            if x[0,iii] < self.norm_min[iii]:
                self.norm_min[iii] = x[0,iii]
       ...
       # self.norm_min[x < self.norm_min] = x[x < self.norm_min]
        ...
        self.hbias = self.hbias + self.params.lr * L_hbias
        self.vbias = self.vbias + self.params.lr * L_vbias
    ...
    def save(self):
        ...
        numpy.save(("model//"+str(self.name)+"thvbias.npy"), self.vbias)
        numpy.save(("model//"+str(self.name)+"thn.npy"), self.n)
        numpy.save(("model//"+str(self.name)+"thname.npy"), self.name)
        numpy.save(("model//"+str(self.name)+"thnorm_max.npy"), self.norm_max)
        numpy.save(("model//"+str(self.name)+"thnorm_min.npy"), self.norm_min)
        numpy.save(("model//"+str(self.name)+"th"), self.params)
    ...
    def load(self):
        ...
        self.vbias = numpy.load(("model//"+str(self.name)+"thvbias.npy"))